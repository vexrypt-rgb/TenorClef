...
public class RunToNode {
	...
	public static Node generateMove(Node parent, WorldView world, BlockNode nextBlockNode) {
		...
		if (world == null) {
			Debug.logMessage("[RunToNode] world null — cannot simulate move");
			return parent;
		}
...
//			desiredYaw = (float) DirectionHelper.calcYawFromVec3d(newNode.agent.getPos(), nextBlockNode.getPos(true));
        ...
        if (disp < 1.0E-4) {
            Debug.logMessage("[RunToNode] near-zero displacement after sim (disp=" + disp + ") parent=" + parent.agent.getPos() + " — check collisions/world");
        }
        return newNode;
	}
}
