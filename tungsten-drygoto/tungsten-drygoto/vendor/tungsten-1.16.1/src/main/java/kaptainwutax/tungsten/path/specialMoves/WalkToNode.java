...
public class WalkToNode {
	...
	public static Node generateMove(Node parent, WorldView world, BlockNode nextBlockNode) {
		...
		if (world == null) {
			Debug.logMessage("[WalkToNode] world null — cannot simulate move");
			return parent;
		}
...
//			desiredYaw = (float) DirectionHelper.calcYawFromVec3d(newNode.agent.getPos(), nextBlockNode.getPos(true));
        ...
        if (disp < 1.0E-4) {
            Debug.logMessage("[WalkToNode] near-zero displacement after sim (disp=" + disp + ") parent=" + parent.agent.getPos() + " — check collisions/world");
        }
        return newNode;
	}
}
